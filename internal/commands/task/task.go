...
func newRestartCommand() *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		RunE: func(_ *cobra.Command, _ []string) error {
			body, err := taskIDBody(taskID)
			if err != nil {
				return shared.EmitLocalError("gm task restart", "INVALID_ARGUMENT", err.Error(), "")
			}
			return shared.CallAPI("gm task restart", "POST", "/task/restart", body, nil)
		},
	}
	cmd.Flags().StringVar(&taskID, "task-id", "", "task id")
	return cmd
...
func newLogsCommand() *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		RunE: func(_ *cobra.Command, _ []string) error {
			...
			if !follow {
				return shared.CallAPI("gm task logs", "POST", "/task/console/log", body, nil)
			}
			if interval <= 0 {
			...
			for {
				if err := shared.CallAPI("gm task logs", "POST", "/task/console/log", body, nil); err != nil {
					return err
				}
